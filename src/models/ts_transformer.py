...
def model_factory(config, data):
    ...
    if (task == "classification") or (task == "regression") or (task == "regression_RMSE"):
        ...
        if config['model'] == 'LINEAR':
            ...
            return TSTransformerEncoderClassiregressor(feat_dim, max_seq_len, config['d_model'],
                                                        ...
                                                        activation=config['activation'],
                                                        norm=config['normalization_layer'], freeze=config['freeze'])
        elif config['model'] == 'original_transformer':
            return TSTransformerEncoderClassiregressorOriginal(feat_dim, max_seq_len, config['d_model'],
                                                        config['num_heads'],
                                                        config['num_layers'], config['dim_feedforward'],
                                                        num_classes=num_labels,
                                                        dropout=config['dropout'], pos_encoding=config['pos_encoding'],
...
class TransformerBatchNormEncoderLayer(nn.modules.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        # Variable for attention mapping
        #self.self_attn, self.self_attn_weights = MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        #squeezed = weights.cpu().squeeze().detach().numpy()
        #squeezed_mean = np.mean(squeezed, axis=0)
        #data = np.exp(squeezed_mean) / np.sum(np.exp(squeezed_mean), axis=0)
        ...
        #    column_index.append(column_name)
        #df = pd.DataFrame(data, columns=['Pred'])
        #df = pd.DataFrame(data, columns=column_index)
        #df.to_csv('weights.txt', header=None, index=False, sep=' ', mode='a')
...
class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def forward(self, X, padding_masks):
        ...
        # JJW: make output text file
        # Be sure to comment after use! Otherwise, training task will be affected
        #print(output)

        #import numpy as np
        #f = open("pred_values.txt", "a")
...
class TSTransformerEncoderClassiregressorOriginal(nn.Module):
    """
    Simplest classifier/regressor. Can be either regressor or classifier because the output does not include
    softmax. Concatenates final layer embeddings and uses 0s to ignore padding embeddings in final output layer.
    """
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
                 dropout=0.1, pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False):
        super(TSTransformerEncoderClassiregressorOriginal, self).__init__()
        ...
        decoder_layer = torch.nn.TransformerDecoderLayer(d_model, self.n_heads, dim_feedforward, dropout*(1.0 - freeze), activation=activation)
