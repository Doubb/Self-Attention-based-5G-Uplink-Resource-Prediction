...
class TSRegressionArchive(BaseData):
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        # JJW
        index_arr = np.arange(len(self.all_df))
        for i in range(len(index_arr)):
            index_arr[i] = int(index_arr[i] / 50)

        index_series = pd.Index(index_arr)
        self.all_df = self.all_df.set_index(index_series)
    ...
    def load_all(self, root_dir, file_list=None, pattern=None):
        ...
        if self.n_proc > 1:
            ...
            logger.info("Loading {} datasets files using {} parallel processes ...".format(len(input_paths), _n_proc))
            with Pool(processes=_n_proc) as pool:
                all_df = pd.concat(pool.map(TSRegressionArchive.load_single2, input_paths))
                labels_df = pd.concat(pool.map(TSRegressionArchive.load_label, input_paths))

            #labels_df = all_df[['ylabel']]
            #print('labels before dropna')
            #print(labels_df.shape)
            #labels_df = labels_df.dropna()
            #print('labels after dropna')
            #print(labels_df.shape)
    ...
    def load_single2(filepath):
        ...
        #labels_df = pd.DataFrame(labels, dtype=np.float32, columns=['ylabel'])
        #print(lengths[0, 0])
        df = pd.concat((pd.DataFrame({col: df.loc[row, col] for col in df.columns}).reset_index(drop=True).set_index(
            pd.Series(lengths[row, 0]*[row])) for row in range(df.shape[0])), axis=0)
        #print('df before concat with labels')
        #print(df.shape)
        #df = pd.concat([df, labels_df])
        #print('df after concat with labels')
        #print(df.shape)
    ...
    def load_label(filepath):
        _, labels = utils.load_from_tsfile_to_dataframe(filepath, return_separate_X_and_y=True, replace_missing_vals_with='NaN')
    ...
    def load_single(self, filepath):
        ...
        if (self.config['task'] == 'regression') or (self.config['task'] == 'regression_RMSE'):
            df, labels = utils.load_from_tsfile_to_dataframe(filepath, return_separate_X_and_y=True, replace_missing_vals_with='NaN')
            labels_df = pd.DataFrame(labels, dtype=np.float32)
