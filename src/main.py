...
def main(config):
    ...
    logger.info("{} samples may be used for training".format(len(my_data.all_IDs)))
    ...
    if config['test_pattern']:  # used if test data come from different files / file patterns
        logger.info("test_pattern : Loading from TEST file ...")
    ...
    if config['val_pattern']:  # used if val data come from different files / file patterns
        logger.info("val_pattern : Loading from VAL file ...")
    ...
    if config['val_ratio'] > 0:
        logger.info("val_ratio is larger than 0. Splitting dataset ...")
    ...
    normalizer = None
    if config['norm_from']:
        logger.info("norm_from : Getting normalization values ...")
        with open(config['norm_from'], 'rb') as f:
            norm_dict = pickle.load(f)
        normalizer = Normalizer(**norm_dict)
    elif config['normalization'] is not None:
        logger.info("normalization : Calculating normalization ...")
    ...
    if normalizer is not None:
        if len(val_indices):
            logger.info("Normalize val_data ...")
            val_data.feature_df.loc[val_indices] = normalizer.normalize(val_data.feature_df.loc[val_indices])
        if len(test_indices):
            logger.info("Normalize test_data ...")
            test_data.feature_df.loc[test_indices] = normalizer.normalize(test_data.feature_df.loc[test_indices])

    # JJW: Pre-process labels
    '''
    logger.info("normalization_label : Calculating normalization ...")
    normalizer_label = Normalizer("minmax")
    my_data.labels_df.loc[train_indices] = normalizer_label.normalize(my_data.labels_df.loc[train_indices])
    norm_dict_label = normalizer_label.__dict__
    with open(os.path.join(config['output_dir'], 'normalization_label.pickle'), 'wb') as f:
        pickle.dump(norm_dict_label, f, pickle.HIGHEST_PROTOCOL)

    logger.info("Normalize label of val_data ...")
    val_data.labels_df.loc[val_indices] = normalizer_label.normalize(val_data.labels_df.loc[val_indices])
    logger.info("Normalize label of test_data ...")
    test_data.labels_df.loc[test_indices] = normalizer_label.normalize(test_data.labels_df.loc[test_indices])
    '''
    ...
    summary(model, [(128, 50, 8), (128, 50)], dtypes=[torch.float, torch.int])
    logger.info("Model Summary:\n{}".format(summary(model, [(128, 50, 8), (128, 50)], dtypes=[torch.float, torch.int])))
    '''
    ...
    logger.info("Making val_data generator ...")
    dataset_class, collate_fn, runner_class = pipeline_factory(config)
    ...
    logger.info("Making train_data generator ...")
    ...
    torch.save(model, '220702_4G5G_Seperated.pt')
