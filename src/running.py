...
def pipeline_factory(config):
    ...
    if (task == "classification") or (task == "regression") or (task == "regression_RMSE"):
        if model == "original_transformer":
            return ClassiregressionDataset, collate_superv, SupervisedRunnerOriginal
        elif model == "measure":
            return ClassiregressionDataset, collate_superv, SupervisedRunnerMeasure
        else:
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    # avg_val_sample_time = avg_val_time / len(val_evaluator.dataloader.dataset_train)
...
class SupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True, testing=False):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            #file_loss = open("losses_downtown_2.txt", "a")
            #file_loss.write(str(mean_loss)+"\n")
            #file_loss.write(str(predictions)+"\n")

            #file_y = open("y_downtown_2.txt", "a")
            #file_y.write(str(targets)+"\n")
...
class SupervisedRunnerMeasure(BaseRunner):

    def __init__(self, *args, **kwargs):

        super(SupervisedRunnerMeasure, self).__init__(*args, **kwargs)
...
class SupervisedRunnerOriginal(BaseRunner):

    def __init__(self, *args, **kwargs):

        super(SupervisedRunnerOriginal, self).__init__(*args, **kwargs)
